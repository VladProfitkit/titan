...
$SectList = CIBlockSection::GetList($arSort, array("IBLOCK_ID"=>27,"ACTIVE"=>"Y") ,false, array("ID","IBLOCK_ID","IBLOCK_TYPE_ID","IBLOCK_SECTION_ID","CODE","SECTION_ID","NAME","SECTION_PAGE_URL"));
while ($SectListGet = $SectList->GetNext())
{
    $arResult['NEWCATEGORY'][]=$SectListGet;
}   
...
$arResult['NEWCATEGORY'] = getSectionList(array("IBLOCK_ID"=>27),array("ID","NAME"));
*/
...
<yml_catalog date="<?= $arResult["DATE"] ?>">
        <shop>
            ...
            <categories>
                <? foreach ($section['CHILDS'] as $arCategory): ?>
                ...
                <? foreach ($arResult["OFFER"] as $arOffer): ?>
                    <offer id="<?= $arOffer["ID"] ?>" available="<?= $arOffer["AVAIBLE"] ?>"<? if (!empty($arOffer["BID"])): ?> bid="<?= $arOffer["BID"] ?>"<? endif ?><? if (!empty($arOffer["CBID"])): ?> cbid="<?= $arOffer["CBID"] ?>"<? endif ?><? if (!empty($arOffer["FEE"])): ?> fee="<?= $arOffer["FEE"] ?>"<? endif ?>>
                        ...
                        <? foreach ($arParams["PARAMS"] as $k => $v):
                            if ($arOffer["DISPLAY_PROPERTIES"][$v]["DISPLAY_VALUE"] && !in_array($k, $arResult["FOR_DELETE"])): ?>
    ...
    if (!function_exists("itemsCycle"))
    {
        ...
        function itemsCycle(&$savedXML, $arResult, $arParams) {
            foreach ($arResult["OFFER"] as $arOffer):
            	$category_id_show;
            	if($arOffer['CONDITION_PROPERTIES']['SECTION_PRODUCT']['VALUE']) // если товар привязан к разделу
            	{
            	     $CATEGID = "";
            	     $arFilter = Array('IBLOCK_ID'=>23,'ID'=>$arOffer['CONDITION_PROPERTIES']['SECTION_PRODUCT']['VALUE']);
            	     $db_list = CIBlockSection::GetList(Array(), $arFilter, false, Array("UF_CATEGORY_SET_YMAR"));
            	     if($uf_value = $db_list->GetNext()){
            	         $CATEGID=$uf_value["UF_CATEGORY_SET_YMAR"];
            	     }
            	     if($CATEGID)
            	     {
            	          $category_id_show = $CATEGID;
            	     }
            	     else
            	     {
								$category_id_show = substr($arOffer["CATEGORY"], 2);
            	     }
            	}
            	else // если товар не привязан к разделу
            	{       
            	    $CATEGID = "";
            	     $IBLOCKIDCATEGORY = substr($arOffer["CATEGORY"], 2);
            	    $arFilter = Array('IBLOCK_ID'=>23,'ID'=>$IBLOCKIDCATEGORY);
            	    $db_list = CIBlockSection::GetList(Array(), $arFilter, false, Array("UF_CATEGORY_SET_YMAR"));
            	    if($uf_value = $db_list->GetNext()){
            	        $CATEGID=$uf_value["UF_CATEGORY_SET_YMAR"];
            	    } 
            	...
            	if($category_id_show != 650 && $category_id_show != 636 && $category_id_show != 638 && $category_id_show != 639 && $category_id_show != 640 && $category_id_show != 641 && $category_id_show != 643 && $category_id_show != 644 && $category_id_show != 645 && $category_id_show != 646 && $category_id_show != 647){
            		continue;
            	}

            	

					$pos = strripos($arOffer["MODEL"],"Экран фронтальный");
					if ($pos === false) {
             	...
             	$pos = strripos($arOffer["MODEL"],"Каркас");
					if ($pos === false) {
					...
					$pos = strripos($arOffer["MODEL"],"Фронтальная панель");
					if ($pos === false) {
					...
					$pos = strripos($arOffer["MODEL"],"Душевой гарнитур");
					if ($pos === false) {
					...
					$pos = strripos($arOffer["MODEL"],"экран");
					if ($pos === false) {
               ...
               if($arOffer['CONDITION_PROPERTIES']['SECTION_PRODUCT']['VALUE']) // если товар привязан к разделу
               {
                    ...
                    if($CATEGID)
                    {
                        $savedXML .= '<categoryId>'.$CATEGID.'</categoryId>';
                    }
                    else
                    {
                        $savedXML .= '<categoryId>'.substr($arOffer["CATEGORY"], 2).'</categoryId>';
                    }
               }
               else // если товар не привязан к разделу
               {       
    ...
    if (!function_exists("xmlHeader"))
    {
        ...
        function xmlHeader(&$savedXML, $arResult, $arParams,$server_name) {
            ...
            $savedXML .= '</currencies>';
            $savedXML .= '<categories>';
            foreach ($arResult['NEWCATEGORY'] as $arCategory):
                $savedXML .= '<category id="' . $arCategory["ID"] . '"';
                if ($arCategory["IBLOCK_SECTION_ID"])
                    $savedXML .= ' parentId="' . $arCategory['IBLOCK_SECTION_ID'] . '"';
