        ...
        <shop>
            ...
            <categories>
                <? foreach ($arResult["CATEGORIES"] as $arCategory): ?>
                    <?if($arCategory["ID"] == 23) continue;?>

                    <?if($arCategory['PARENT'] == 23)
                    {?>
                        <category id="<?= $arCategory["ID"] ?>"><?= $arCategory["NAME"] ?></category>
                    <?}
                    else
                    {?>
    ...
    if (!function_exists("itemsCycle"))
    {
        ...
        function itemsCycle(&$savedXML, $arResult, $arParams) {
            foreach ($arResult["OFFER"] as $arOffer):
                ...
                if($arOffer['CONDITION_PROPERTIES']['SECTION_PRODUCT']['VALUE'])
                {
                    $savedXML .= '<categoryId>23'.$arOffer['CONDITION_PROPERTIES']['SECTION_PRODUCT']['VALUE'].'</categoryId>';
                }
                else
                {                 
