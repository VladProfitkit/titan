...
if((!empty($sotbitSeoMetaTopDesc)) or (!empty($sotbitSeoMetaTitle)) or (!empty($sotbitSeoMetaH1)) or (strpos($APPLICATION->GetCurPage(), 'filter'))){
?>
...
<?if($itemsCnt):?>
			...
			<?}?>
						   <?
			              $idElementSectionFilter = "";
			              $arFilter = Array('IBLOCK_ID'=>$arParams["IBLOCK_ID"], 'ID'=>$arResult["VARIABLES"]["SECTION_ID"]);
			              $db_list = CIBlockSection::GetList(array(), $arFilter, true, array("UF_CATEGORY_FILTER"));
			              if ($ar_result = $db_list->GetNext())
			              {
			                if ($ar_result['UF_CATEGORY_FILTER']) {
			                  $idElementSectionFilter = $ar_result['UF_CATEGORY_FILTER'];
			                }
			              }
			              if($idElementSectionFilter) {
			                $VALUES = array();
			                $res = CIBlockElement::GetProperty(26, $idElementSectionFilter, "sort", "asc", array("CODE" => "CATEGORY_SHOW"));
			                while ($ob = $res->GetNext()) {
			                  $VALUES[] = $ob;
			                }
			                $arResult['SECTION_SHOW'] = $VALUES;
			              }
			              ?>
			              <?if($arResult['SECTION_SHOW']){?>
			                  <div class="category-filter">
			                      <span class="">Популярные категории: </span>

			                    <?$x=0;
			                    foreach ($arResult['SECTION_SHOW'] as $key => $item) {?>
			                      <?if($key<=9){?>
			                            <a class="" href="<?=$item['VALUE']?>"><?=$item['DESCRIPTION']?></a>
			                      <?}else{
			                        $x++;?>
			                            <a class="hidden-category" href="<?=$item['VALUE']?>"><?=$item['DESCRIPTION']?></a>
			                      <?}?>
			                    <?}
			                    if($x!=0){?>
			                        <div class="show-category">Еще</div>
			                    <?}?>
			                  </div>
			              <?}?>
...
$UFArr = CIBlockSection::GetList(
    false,
    Array("IBLOCK_ID" => 23, "ID" => $arSection['ID'], "ACTIVE"=>"Y", "GLOBAL_ACTIVE"=>"Y", "SECTION_ACTIVE" => "Y"),
    false,
    Array("UF_H1"),
    false
);
$UFValue = $UFArr->Fetch();
if ($UFValue['UF_H1']) {
    $APPLICATION->SetTitle($UFValue['UF_H1']);
}
