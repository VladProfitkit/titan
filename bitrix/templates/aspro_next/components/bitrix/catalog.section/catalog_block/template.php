					...
					<div class="catalog_item main_item_wrapper item_wrap <?=(($_GET['q'])) ? 's' : ''?>" id="<?=$arItemIDs["strMainID"];?>">
						<div>
							<div class="image_wrapper_block">
								...
								<? if($arItem['PROPERTIES']['ARCHIV_TOV']['VALUE'] != 'Да'): ?>
							...
							<div class="item_info <?=$arParams["TYPE_SKU"]?>">
								...
								<?if($arItem['PROPERTIES']['STRANA']["VALUE"]){?><div style="font-size: 15px;color: #000;    margin-bottom: 4px;">Страна: <?=$arItem['PROPERTIES']['STRANA']["VALUE"]?></div><?}?>
								...
								<? if($arItem['PROPERTIES']['ARCHIV_TOV']['VALUE'] != 'Да'): ?>
									<?=$arQuantityData["HTML"];?>
								<? else: ?>
								<div class="item-stock archive"><span class="value">Архив</span></div>
								<? endif; ?>		
                                ...
                                <? else: ?>
                                <span class="list_archive_info">Товар снят с производства</span>
                                <? endif; ?>
							...
							<div class="params-info">
								<?if($arItem['PROPERTIES']['NAZNACHENIE']["VALUE"]){?><div>Назначение: <?=$arItem['PROPERTIES']['NAZNACHENIE']["VALUE"]?></div><?}?>
								<?if($arItem['PROPERTIES']['FORMA_IZLIVA']["VALUE"]){?><div>Форма излива: <?=$arItem['PROPERTIES']['FORMA_IZLIVA']["VALUE"]?></div><?}?>
								<?if($arItem['PROPERTIES']['STANDART_PODVODKI']["VALUE"]){?><div>Стандарт подводки: <?=$arItem['PROPERTIES']['STANDART_PODVODKI']["VALUE"]?></div><?}?>
							</div>
						</div>
					</div>
				</div>
